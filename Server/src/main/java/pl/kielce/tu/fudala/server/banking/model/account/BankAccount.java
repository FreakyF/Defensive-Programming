...
public class BankAccount implements IBankAccount {
    ...
    private BankAccount castToBankAccount(IBankAccount account) {
        if (!(account instanceof BankAccount)) {
            throw new InvalidTransactionException("Target account must be an instance of BankAccount.");
        }
    ...
    private void ensureSufficientFunds(BigDecimal requiredAmount) {
        if (this.balance.compareTo(requiredAmount) < 0) {
            throw new InsufficientFundsException(this.accountNumber, "Transfer",
                    "Insufficient funds on source account. Required: " + requiredAmount +
    ...
    private void processWithdrawal(BigDecimal amount) {
        if (balance.compareTo(amount) < 0) {
            throw new InsufficientFundsException(this.accountNumber, "Withdrawal",
    ...
    private void validateAmount(BigDecimal amount, OperationType operationType) {
        if (amount == null) {
            throw new InvalidTransactionException(operationType + " amount must not be null.");
        }
        if (amount.compareTo(BigDecimal.ZERO) <= 0) {
            throw new InvalidTransactionException(operationType + " amount must be greater than zero.");
        }
    }

    private void validateTargetAccount(IBankAccount targetAccount) {
        if (targetAccount == null) {
            throw new InvalidTransactionException("Target account must not be null.");
        }
