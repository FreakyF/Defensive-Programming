...
import pl.kielce.tu.fudala.client.banking.model.transaction.OperationType;
...
public class BankAccount implements IBankAccount {
	...
	private void ensureSufficientFunds(BigDecimal requiredAmount) {
		if (this.balance.compareTo(requiredAmount) < 0) {
			throw new InsufficientFundsException(
					"Insufficient funds on account " + this.accountNumber +
							" for Transfer. Required: " + requiredAmount +
	...
	private void processWithdrawal(BigDecimal amount) {
		if (balance.compareTo(amount) < 0) {
			throw new InsufficientFundsException(
					"Insufficient funds on account " + this.accountNumber +
							" for Withdrawal. Insufficient funds for withdrawal.");
		}
		this.balance = this.balance.subtract(amount);
